...
export default internalAction({
  ...
  handler: async (ctx, { delayMs = 8_000 }) => {
    ...
    while (true) {
      ...
      // Read once at the deadline so the final result includes the last wait.
      if ((delayed && retriesDone) || Date.now() >= deadline) break;
      await new Promise((r) => setTimeout(r, pollMs));
    }

    const delayed = probes.find((p) => p.label === "delayed");
    const ranEarly = !!delayed && delayed.at < enqueuedAt + delayMs;
